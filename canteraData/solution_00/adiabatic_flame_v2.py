...
cbDict = commonblock.load_commBlk()
...
def adiabaticFlame(solIdx,cbDict):
    ...
    #  print('multicomponent flamespeed = {0:7f} m/s'.format(f.u[0]))
      ...
      data[:,cbDict['nVarCant']:cbDict['nSpeMech']+cbDict['nVarCant']] 
          = np.transpose(f.Y) # store species mass fractions
      ...
      if abs(Tin - 298.15) < 0.1:
        ...
        for kk in range(1,int(100*(Tin-298.15))):
    ...
    if abs(T_fuel - 298.15) < 0.1:
      ...
      for kk in range(1,int(100*(T_fuel-298.15))):
    ...
    BCdata[0,5] = gas_fuel.viscosity/gas_fuel.density
    ...
    if abs(T_ox - 298.15) < 0.1:
      ...
      for kk in range(1,int(100*(T_ox-298.15))):
    ...
    BCdata[1,5] = gas_ox.viscosity/gas_ox.density
